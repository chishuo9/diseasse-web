...
def get_tencent_data():
    ...
    data1_all = json.loads(d1['data'])

    res2 = requests.get(url2, headers)
    d2 = json.loads(res2.text)
    data2_all = json.loads(d2['data'])

    history = {}  # 历史数据

    for i in data2_all['chinaDayList']:
        ds = '2020.' + i['date']
    ...
    for i in data2_all['chinaDayAddList']:
        ds = '2020.' + i['date']
        tup = time.strptime(ds, '%Y.%m.%d')
        ds = time.strftime('%Y-%m-%d', tup)
        confirm = i['confirm']
    ...
    details = []  # 当日详细数据
    update_time = data1_all['lastUpdateTime']
    data_China = data1_all['areaTree']
    data_province = data1_all['areaTree'][0]['children']
